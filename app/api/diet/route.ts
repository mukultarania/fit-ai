...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { weight, height, age, gender, activityLevel, dietaryPreference, goal, allergies, medicalConditions } = body;
  ...
  "restrictions": [
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are a professional nutritionist creating personalized diet plans. Provide detailed, safe, and effective meal plans based on the individual's needs and restrictions. Always respond with valid JSON that matches the specified structure."
        },
        {
          role: "user",
          content: prompt
